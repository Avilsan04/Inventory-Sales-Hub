...
const checkoutSchema = z.object({
    ...
    if (!isExpiryValid(data.expiry ?? ''))
        ctx.addIssue({ code: 'custom', path: ['expiry'], message: 'Invalid or expired date (MM/YY)' });
...
type FieldPath = keyof FormValues | `items.${number}.productId` | `items.${number}.quantity` | `items.${number}.unitPrice`;
...
const STEPS = ['sales.checkout.stepItems', 'sales.checkout.stepShipping', 'sales.checkout.stepPayment', 'sales.checkout.stepConfirm'];

// ── Stepper component ─────────────────────────────────────────────────────────

function Stepper({ current, translate }: { current: number; translate: (k: string) => string }): React.ReactElement {
    return (
        <div className={styles['stepper']}>
            {STEPS.map((labelKey, i) => (
                <React.Fragment key={labelKey}>
                    {i > 0 && (
                        <div className={cn(styles['stepConnector'], i <= current && styles['stepConnectorDone'])} />
                    )}
...
export function SaleCreateDialog({ open, onOpenChange }: Props): React.ReactElement {
    ...
    const {
        ...
        reset, control, setValue, watch, trigger,
    } = useForm<FormValues>({
    ...
    const onClose = (): void => { reset(); setStep(0); onOpenChange(false); };
    ...
    const onSubmit = (data: FormValues): void => {
        mutate(
            ...
            {
                onSuccess: () => {
                    toast({ title: t('sales.checkout.orderPlaced'), description: t('sales.checkout.orderPlacedDesc') });
                    onClose();
                },
                onError: (err) => {
    ...
    const renderStep0 = (): React.ReactElement => (
        <div className={styles['bodyScroll']}>
                    ...
                    <Controller
                        ...
                        render={({ field }) => (
                            <Select value={field.value ?? ''} onValueChange={field.onChange}>
                                <SelectTrigger><SelectValue placeholder={t('sales.checkout.walkIn')} /></SelectTrigger>
                                <SelectContent>
                                    {customers?.map((c) => (
                ...
                {fields.map((field, index) => {
                    ...
                    return (
                        <div key={field.id} className={styles['saleItemRow']}>
                            <FormField label={index === 0 ? t('sales.checkout.product') : ''} error={row?.productId?.message}>
                                <Controller
                                    ...
                                    render={({ field: f }) => (
                                        ...
                                        >
                                            <SelectTrigger><SelectValue placeholder={t('sales.checkout.selectProduct')} /></SelectTrigger>
                                            <SelectContent>
                                                {products?.map((p) => (
                            ...
                            <FormField label={index === 0 ? t('sales.checkout.qty') : ''} error={row?.quantity?.message}>
                                {/* eslint-disable-next-line @typescript-eslint/restrict-template-expressions */}
                                <Input {...register(`items.${index}.quantity`, { valueAsNumber: true })} type="number" min="1" step="1" />
                            </FormField>
                            <FormField label={index === 0 ? t('sales.checkout.price') : ''} error={row?.unitPrice?.message}>
                                {/* eslint-disable-next-line @typescript-eslint/restrict-template-expressions */}
                                <Input {...register(`items.${index}.unitPrice`, { valueAsNumber: true })} type="number" min="0" step="0.01" />
                            </FormField>
                            <Button
            ...
            <div className={styles['runningTotal']}>
                ...
                <strong>{runningTotal.toFixed(2)} {watch('currency')}</strong>
            </div>
        </div>
    );
        ...
        <div className={styles['body']}>
            ...
            <div className={styles['grid2']}>
                <FormField label={t('sales.checkout.contactName')} error={errors.contactName?.message}>
                    <Input {...register('contactName')} placeholder={t('sales.checkout.contactNamePlaceholder')} />
                </FormField>
                <FormField label={t('sales.checkout.contactPhone')} error={errors.contactPhone?.message}>
                    <Input {...register('contactPhone')} placeholder={t('sales.checkout.contactPhonePlaceholder')} />
                </FormField>
            </div>
            ...
            <div className={styles['paymentCards']}>
                {PAYMENT_OPTIONS.map(({ method, icon, labelKey }) => (
                    <button
                        key={method}
                        type="button"
                        className={cn(styles['paymentCard'], watchedMethod === method && styles['paymentCardActive'])}
            ...
            {watchedMethod === 'credit_card' && (
                <>
                    <FormField label={t('sales.checkout.holderName')} error={errors.holderName?.message}>
                        <Input {...register('holderName')} placeholder={t('sales.checkout.holderNamePlaceholder')} />
                    </FormField>
                    ...
                    <div className={styles['grid2']}>
                        ...
                        <FormField label={t('sales.checkout.cvv')} error={errors.cvv?.message}>
                            <Input {...register('cvv')} placeholder={t('sales.checkout.cvvPlaceholder')} maxLength={4} type="password" />
                        </FormField>
                    </div>
                </>
            )}
    ...
    const renderStep3 = (): React.ReactElement => (
        <div className={styles['body']}>
            ...
            <div className={styles['summaryRow']}>
                ...
                <strong className={styles['summaryValue']}>{runningTotal.toFixed(2)} {formValues.currency}</strong>
            </div>

            <div className={styles['summaryRow']}>
                <span className={styles['summaryLabel']}>{t('sales.checkout.shippingTo')}</span>
                <span className={styles['summaryValue']}>
                    {selectedCustomerName ? `${selectedCustomerName} — ` : ''}{formValues.address}
                </span>
            </div>
            ...
            <div className={styles['summaryRow']}>
                ...
                <span className={styles['summaryValue']}>
                    ...
                    {formValues.paymentMethod === 'credit_card' && formValues.cardNumber ? ` — ${maskedCard}` : ''}
                </span>
            </div>
