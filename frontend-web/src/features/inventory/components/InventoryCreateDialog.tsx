...
export function InventoryCreateDialog({ open, onOpenChange }: Props): React.ReactElement {
    ...
    const onSubmit = (data: FormValues): void => {
        mutate(data, {
            onSuccess: () => { toast({ title: 'Item added to inventory' }); onClose(); },
            onError: (err) => { toast({ title: 'Failed to add item', description: err.message, variant: 'destructive' }); },
        });
    };
                    ...
                    <div className={styles['body']}>
                        ...
                        <div className={styles['gridQtyPriceShort']}>
                            <FormField label="Quantity" required error={errors.quantity?.message}>
                                <Input {...register('quantity', { valueAsNumber: true })} type="number" min="0" step="1" />
                            </FormField>
                            <FormField label="Price" required error={errors.price?.message}>
                                <Input {...register('price', { valueAsNumber: true })} type="number" step="0.01" min="0" />
                            </FormField>
