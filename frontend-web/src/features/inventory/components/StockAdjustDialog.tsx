...
export function StockAdjustDialog({ item, open, onOpenChange }: Props): React.ReactElement {
    ...
    const onSubmit = (data: FormValues): void => {
        if (item === null) return;
        mutate(data, {
            onSuccess: () => { toast({ title: 'Stock adjusted' }); onClose(); },
            onError: (err) => { toast({ title: 'Adjustment failed', description: err.message, variant: 'destructive' }); },
        });
    };
            ...
            <DialogContent>
                ...
                <form onSubmit={(e: React.SyntheticEvent) => { void handleSubmit(onSubmit)(e); }}>
                    ...
                    <DialogFooter>
                        ...
                        <Button type="submit" disabled={isPending}>{isPending ? 'Adjusting…' : 'Apply'}</Button>
                    </DialogFooter>
                </form>
            </DialogContent>
        </Dialog>
