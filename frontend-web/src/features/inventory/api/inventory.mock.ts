...
export const inventoryHandlers = [
  http.get(`${API_BASE_URL}/inventory`, async () => {
    await delay(800);
    return HttpResponse.json(mockInventory.filter((i) => i.isActive));
  }),

  http.get(`${API_BASE_URL}/inventory/low-stock`, async () => {
    await delay(600);
    return HttpResponse.json(mockInventory.filter((i) => i.isActive && i.status !== 'IN_STOCK'));
  }),
  ...
  http.patch(`${API_BASE_URL}/inventory/:id`, async ({ params, request }) => {
    ...
    if ('is_active' in body) {
      mockInventory[idx] = { ...existing, isActive: body.is_active ?? true };
