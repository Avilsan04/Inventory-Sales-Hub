...
function roleVariant(role: EmployeeRole): BadgeVariant {
    const map: Record<EmployeeRole, BadgeVariant> = { admin: 'default', manager: 'secondary', staff: 'outline' };
    return map[role];
}
            ...
            <section className={statsStyles['statsRow']} aria-label="Employee statistics">
                <Card>
                    <CardHeader>
                        ...
                        <CardAction><span className={statsStyles['statIcon']}><UserCogIcon aria-hidden="true" /></span></CardAction>
                    </CardHeader>
                    <CardContent><div className={statsStyles['statValue']}>{data.length}</div></CardContent>
                </Card>
                <Card>
                    <CardHeader>
                        <CardTitle className={statsStyles['statTitle']}>{t('employees.activeEmployees')}</CardTitle>
                        <CardAction><span className={statsStyles['statIcon']}><CheckCircle2Icon aria-hidden="true" /></span></CardAction>
                    </CardHeader>
                    <CardContent><div className={statsStyles['statValue']}>{activeCount}</div></CardContent>
                </Card>
                <Card>
                    <CardHeader>
                        <CardTitle className={statsStyles['statTitle']}>{t('employees.adminCount')}</CardTitle>
                        <CardAction><span className={statsStyles['statIcon']}><ShieldCheckIcon aria-hidden="true" /></span></CardAction>
                    </CardHeader>
                    <CardContent><div className={statsStyles['statValue']}>{adminCount}</div></CardContent>
                </Card>
            </section>
                                        ...
                                        <TableRow key={e.id}>
                                            ...
                                            <TableCell>
                                                <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                                                    <Button variant="ghost" size="icon-sm" onClick={() => { setEditEmployee(e); }}>
                                                        <PencilIcon size={14} />
                                                    </Button>
                                                </div>
                                            </TableCell>
