...
function StatusTooltip({ active, payload }: TooltipContentProps): React.ReactElement | null {
    ...
    return (
        <div style={{
            ...
            <p style={{ margin: '0 0 4px', fontSize: 12, fontWeight: 600, color: 'var(--color-text-primary)', textTransform: 'capitalize' }}>
                {slice.status}
            </p>
...
function renderCustomLabel({ cx, cy, midAngle, innerRadius, outerRadius, percent }: LabelProps): React.ReactElement | null {
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {filtered.map((slice, i) => (
                    <div key={slice.status} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--color-text-primary)', marginLeft: 'auto', paddingLeft: 8 }}>
                            {slice.count}
                        </span>
                    </div>
                ))}
