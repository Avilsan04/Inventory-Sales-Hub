...
export default function CartScreen() {
  const { cart, removeFromCart, increaseQty, decreaseQty, total, itemCount } = useCart();
    ...
    return (
      <CheckoutScreen
        ...
        onSuccess={() => {
          ...
          Alert.alert('✓ Pedido realizado', 'Tu pedido se ha procesado correctamente. ¡Gracias por tu compra!');
        }}
      />
    );
  }
  ...
  return (
    <View style={s.container}>
        ...
        renderItem={({ item }) => (
          <View style={s.item}>
            ...
            <View style={s.controls}>
              <TouchableOpacity style={s.ctrl} onPress={() => decreaseQty(item.id)}>
                <Text style={s.ctrlTxt}>−</Text>
              </TouchableOpacity>
              <Text style={s.qty}>{item.quantity}</Text>
              <TouchableOpacity style={s.ctrl} onPress={() => increaseQty(item.id)}>
                <Text style={s.ctrlTxt}>+</Text>
              </TouchableOpacity>
              <TouchableOpacity style={s.del} onPress={() => removeFromCart(item.id)}>
                <Text>🗑</Text>
              </TouchableOpacity>
            </View>
          </View>
      ...
      <View style={s.footer}>
        ...
        <TouchableOpacity style={s.btn} onPress={() => setShowCheckout(true)}>
          <Text style={s.btnText}>Proceder al pago</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
